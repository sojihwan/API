            ...
            api.data.list.map((item,index) => {
              return(
                <div className="img" key={index} onClick={() => {idgive(api.data.list[index])}}>
                  <img className="bookimg" onClick={model} src={(api.data.list[index].thumbnailUrl != null)?api.data.list[index].thumbnailUrl: api.data.list[index].thumbnailUrl.length===64 'img/ddd.jpg'}
                  /><br/>
